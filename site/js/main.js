...
(function () {
  ...
  var translations = {
    zh: {
      ...
      'feat-mips-desc': 'A320 和 A330 使用缓存解释器，并可在支持的 64 位平台上将热点整数代码块编译为本机代码；无法安全编译的指令会自动回退解释器。',
      'feat-sdk-title': '设备 SDK HLE',
      'feat-sdk-desc': '按设备提供图形、输入、音频、随机访问文件与目录枚举、资源、任务和同步等高层模拟服务，并为无可见末帧的 A330 程序保留退出诊断。',
    ...
    en: {
      ...
      'feat-mips-desc': 'A320 and A330 use cached interpreters and can compile hot integer blocks to native code on supported 64-bit platforms, with automatic fallback for instructions that cannot be translated safely.',
      'feat-sdk-title': 'Device SDK HLE',
      'feat-sdk-desc': 'Device-specific high-level services for graphics, input, audio, random-access files and directory enumeration, resources, tasks, and synchronization, plus visible diagnostics when A330 software exits without a usable final frame.',
